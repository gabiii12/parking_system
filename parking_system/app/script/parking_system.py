    ...
    def insert_data(self):
        if self.status == True:
            try:
                ...
                self.cursor.execute(get_data,(self.tag_id,))
                emp_inf = self.cursor.fetchone()
                ...
                self.cursor.execute(last_action,(self.tag_id,))
                la = self.cursor.fetchone()
                ...
                insert_data = """INSERT INTO parking_logs (card_uid, employee_name, vehicle_plate, last_action)
                            VALUES(%s,%s,%s,%s)
                """
                self.cursor.execute(insert_data,(self.tag_id,employee_name,vehicle_plate,new_status))
                self.db.conn.commit()
                print("INSERTED!")
